...
public class CircleMovementMethod extends BaseMovementMethod {
    ...
    private final static int DEFAULT_COLOR_ID = R.color.cr;//默认背景透明
    private final static int DEFAULT_CLICKABLEA_COLOR_ID = R.color.wh_b;//默认点击灰色
    /**
     * 整个textView的背景色
     */
    ...
    public CircleMovementMethod() {
        this.textViewBgColor = ResourcesUtils.getColor(DEFAULT_COLOR_ID);
        this.clickableSpanBgClor = ResourcesUtils.getColor(DEFAULT_CLICKABLEA_COLOR_ID);
    }
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer,
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            widget.setBackgroundResource(R.color.cr);
        } else {
            if (mBgSpan != null) {//移除点击时设置的背景span
                buffer.removeSpan(mBgSpan);
            }
